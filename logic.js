...
const PASSWORD_CACHE_DURATION = 3 * 60 * 1000; // 3 minutes in milliseconds

function verifyPassword() {
    const now = Date.now();
    const timeSinceLastPassword = now - state.lastPasswordTime;
    
    // If less than 3 minutes have passed, skip password prompt
    if (timeSinceLastPassword < PASSWORD_CACHE_DURATION) {
        return true;
    }
...
export function initCloudSync() {
    ...
    // Increment page views atomically on first load with error handling
    ...
    dbRef.on('value', (snapshot) => {
        ...
        if (data) {
            ...
            saveToCloud().catch(err => {
                console.error("Error in initial cloud save:", err);
            });
        }
    }, (error) => {
        console.error("Firebase Sync Error:", error);
        // Don't throw - gracefully degrade to local storage mode
    });
}
...
export async function saveToCloud() {
    ...
    try {
        ...
        // Don't show toast for connection errors during background saves
        if (state.hasUnsavedChanges) {
...
export function setupEventListeners() {
    ...
    // Animation Toggle
    DOM.animationToggle.addEventListener('click', toggleAnimation);
