...
export function openModal(btnData = null) {
    // Password verification handled by caller (handleButtonClick in admin mode)
    // or verifyPassword in logic.js
...
export function openCategoryModal(cat = null) {
    // Password verification handled by caller (category controls in admin mode)
...
export function applyAnimationState() {
    ...
    const icon = DOM.animationToggle.querySelector('i');
    icon.className = state.isAnimationPlaying ? 'fas fa-pause' : 'fas fa-play';
}
